Dynamics of Efficient Ensemble Coding - Data

Long Ni and Alan A Stocker

MATLAB data structure - The structure contains five sub-fields, specifying discrimination response (dat.resp), ensemble orientations relative to the reference (dat.smSet), generative means of the sampling distributions (dat.cnd), and inter-stimulus interval (dat.isi) on each trial. 

Each sub-field contains 5 (ISI conditions)* 9 (subjects) cells. 
The five inter-stimulus interval conditions are [-900, -450), [-450,0), 0, (0 450], (450, 900] ms. 


The code for implementing Efficient Ensemble coding is under the folder 'model'. 
